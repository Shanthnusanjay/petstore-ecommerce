...
from .models import Pet, Customer, CartItems, Address, Order, OrderItem
...
def add_to_cart(request, pet_id):
    ...
    cart_items = CartItems.objects.filter(customer=curr_customer, pet=pet)
    print(cart_items)
    if len(cart_items) == 0:
        #Empty cart
        cart_items = CartItems.objects.create(customer=curr_customer, pet=pet)
    else:
...
def remove_cart_items(request, pet_id):
    pet = Pet.objects.get(id=pet_id)
